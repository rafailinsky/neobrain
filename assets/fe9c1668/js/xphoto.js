...
var XPhoto = XPhotoClass({
    ...
    raiseEvents: function(){
    	...
    	if(this.options.capture === true){
    		if(this.options.withModal === true){
    			$(".xphoto-buttons .xphoto-button-camera").live('click',function(){    				
	    			obj.buildModal();
	    		});    			
    		}else{
    			$(".xphoto-buttons .xphoto-button-camera").live('click',function(){    				
	    		...
	    		$(".xphoto-buttons .xphoto-button-snap").live('click',function(){
	    			$(this).hide();
    	...
    	if(this.options.upload === true){
    		$("#xphoto-input-file").live('change',function(event){
    			obj.handleImage(event);
    		});
    		$(".xphoto-buttons .xphoto-button-upload").live('click',function(){
    ...
    createButtons: function(){
    	var buttons = '';
    	if(this.options.capture===true){
    		buttons += '<button type="button" class="btn btn-success xphoto-button-camera"><i class="fa fa-camera"></i> Capturar</button>'
					  +'<button type="button" class="btn btn-primary xphoto-button-snap" style="display:none;"><i class="fa fa-circle"></i></button>';
		}
		if(this.options.upload===true){
			buttons += '<button type="button" class="btn btn-default xphoto-button-upload"><i class="fa fa-upload"></i> Adjuntar</button>';
		}
    ...
    buildModal: function(){
					  ...
					  +'<div class="modal-dialog">'
					    +'<div class="modal-content">'
					      +'<div class="modal-header">'
					        ...
					        +'<h4 class="modal-title">Pemotretan</h4>'
					      +'</div>'
		...
		obj.startCapture();
		$(".xphoto-modal-buttons .xphoto-modal-button-snap").live('click',function(){			
			obj.snapPhoto();
